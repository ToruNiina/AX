#define BOOST_TEST_MODULE "test_dynamic_vector"
...
{
    const VectorDd vec(10);

    for(std::size_t i=0; i<10; ++i)
        BOOST_CHECK_EQUAL(vec[i], 0e0);

    const VectorDd vec_1(10, 1e0);

    for(std::size_t i=0; i<10; ++i)
        BOOST_CHECK_EQUAL(vec_1[i], 1e0);

    const VectorDd vec_cp_0(vec_1);

    for(std::size_t i=0; i<10; ++i)
        BOOST_CHECK_EQUAL(vec_cp_0[i], 1e0);

    const VectorDd vec_cp_1 = vec_1;
...
{
    const VectorDd vec1(10, 1e0);
    const VectorDd vec2(10, 2e0);
    const VectorDd vec3(vec1 + vec2);
...
{
    const VectorDd vec1(10, 1e0);
    const VectorDd vec2(2e0 * vec1);

    for(std::size_t i=0; i<10; ++i)
        BOOST_CHECK_EQUAL(vec2[i], 2e0);

    const VectorDd vec3(vec1 * 2e0);
...
{
    const VectorDd vec1(10, 2e0);
    const VectorDd vec2(vec1 / 2e0);
...
BOOST_AUTO_TEST_CASE(VectorNd_dot_product)
{
    ...
    for(auto i=0; i<100; ++i)
    {
        ...
        const VectorDd vec1(v1);
        const VectorDd vec2(v2);
